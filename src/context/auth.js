...
function Auth(props) {
    ...
    const signupFunction = async (user) => {
        try {
            const response = await superagent.post(`https://todobackends.herokuapp.com/signup`, user);
    ...
    const lofInFunction = async (username, password) => {
        try {
            const response = await superagent.post(`https://todobackends.herokuapp.com/signin`).set('authorization', `Basic ${base64.encode(`${username}:${password}`)}`);
    ...
    const validateMyToken = async (input) => {
        ...
        if (token) {
            let validUser = await superagent.post(`https://todobackends.herokuapp.com/user`).set('authorization', `Bearer ${token}`);
            setIsLoggedIn(true);
